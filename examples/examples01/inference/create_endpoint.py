...
def validate_endpoint_config(endpoint_config):
    ...
    required_fields = [
        ...
        "model_format_name", "exit_duration", "priority"
    ]
    ...
    try:
        ...
        for endpoint_config in config.get("inference_endpoints", []):
            try:
                ...
                secret_dict = None
                if endpoint_config.get("secret"):
                    if isinstance(endpoint_config["secret"], dict):
                        secret_dict = endpoint_config["secret"]
                    else:
                        try:
                            secret_dict = json.loads(endpoint_config["secret"])
                ...
                model_args = None
                if endpoint_config.get("args"):
                    if isinstance(endpoint_config["args"], list):
                        model_args = endpoint_config["args"]
                    else:
                        model_args = endpoint_config["args"].split()
                ...
                inference_endpoint = egs.create_inference_endpoint(
                    ...
                    standard_model_spec=ModelSpec(
                        model_format_name=endpoint_config["model_format_name"],
                        storage_uri=endpoint_config.get("storage_uri"),
                        args=model_args,
                        secret=secret_dict,
                        resources=None
                    ),
