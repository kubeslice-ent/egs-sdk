        ...
        for template_config in config.get("gpr_template", []):
            try:
                ...
                if not template_exists(templates.items, template_config["name"]):
                    raise ValueError(f"Template '{template_config['name']}' doesnot exists")
                ...
                # Update the GPR template
                gpr_template = egs.update_gpr_template(
                ...
                print(f"✅ Template Updated successfully: {gpr_template}")

            except Exception as e:
                print(f"❌ Failed to update template '{template_config.get('name', 'unnamed')}': {e}")
                continue
